...
class Solution:
    ...
    # Union find #TODO
    def findAllPeopleUF(self, n: int, meetings: List[List[int]], firstPerson: int) -> List[int]:
        pass

    # Sort by meeting time then BFS. Time out on n=50K
    def findAllPeople(self, n: int, meetings: List[List[int]], firstPerson: int) -> List[int]:
        if not meetings: return []
        last = 0
        for _,_,time in meetings:
            last = max(last, time)
        arr = [None] * (last + 1)
        ...
        peopleSecret = set()
        peopleSecret.add(0)
        peopleSecret.add(firstPerson)
        for graph in arr:
            if not graph:
                continue
            queue = Deque()
            for p in graph:
                if p in peopleSecret:
                    queue.append(p)
            while queue:
                curPerson = queue.popleft()
                for neighbor in graph[curPerson]:
                    if neighbor not in peopleSecret:
                        peopleSecret.add(neighbor)
                        queue.append(neighbor)
        return list(peopleSecret)
